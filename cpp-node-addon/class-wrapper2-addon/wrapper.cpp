...
ObjectWrapDemo::ObjectWrapDemo(const Napi::CallbackInfo& info)
  ...
  this->_hoge = Hoge( info[1].As<Napi::Number>().Uint32Value());
}
...
Napi::Value ObjectWrapDemo::StartThread(const Napi::CallbackInfo& info){
  Napi::Env env = info.Env();
  
  this->_hoge.start_thread();
  return Napi::String::New(env, "Start Thread!!");
}
...
Napi::Function ObjectWrapDemo::GetClass(Napi::Env env) {
  return DefineClass(
      ...
      "ObjectWrapDemo",
      {
          ...
          ObjectWrapDemo::InstanceMethod("start_thread", &ObjectWrapDemo::StartThread),
      });
}
